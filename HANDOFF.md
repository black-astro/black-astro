# 핸드오프 — 언어 가이드 확장 (2026-08-31 19차 갱신)
...
### 이번 회차 — 카탈로그 역적용 10차 (그림 7개 · §3 에 상세)
...
### 3. 🟡 기존 그림에 카탈로그 역적용 — 50개 완료, 계속 하면 됩니다

**끝낸 것 (2026-08-31 · 50개)** — 전부 "의도는 이미 그 모델인데 사각형으로 그려져 있던" 것들입니다.

1~3차 15개(아래 표) + 4차 4개 + 5~8차 각 5개 + 9차 4개 + 10차 7개.
...
```bash
node guide-src/tools/chartcand.mjs            # 전체 (지금 22개)
...
나온 것을 전부 바꿀 필요는 없습니다. 다음 순번으로 볼 만한 것들:

| 후보 | 내용 | 바꿀 모델 |
|---|---|---|
| `server/d03` | 멀티스테이지 800 / 190 / 28MB (§3 표의 d03 과 **다른 그림**입니다) | `bar` — 29배라 실척으로 그려집니다 |
| `java/f11` | 오류 처리 지연 200 / 400 / 800ms | `bar` (두 배씩이라 축이 붙으면 바로 읽힙니다) |
| `js-ts/o11` | CI 파이프라인 2 / 3 / 1분 | `gantt` — "빠르게 실패하는 걸 앞에" 가 요점 |
| `cpp/g02` `kotlin/st01` | 설치 용량 30/5/20GB · 2/10/30GB | `bar` (같은 꼴 둘) |
| `python/q25` | 상태 전이 240 / 300 / 400ms | `bar` |
...
- `db/e01` · `server/x07` · `cs/l08` — 각각 표 예시 · 흐름도 · 인원 세기라 차트가 아닙니다.

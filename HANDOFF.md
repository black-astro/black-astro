# 핸드오프 — 언어 가이드 확장 (2026-08-31 18차 갱신)
...
### 이번 회차 — 카탈로그 역적용 4~9차 (그림 28개 · §3 에 상세)

새로 그린 그림은 없습니다. **이미 있던 그림 28개를 카탈로그 모델로 다시 그렸습니다.**
...
### 3. 🟡 기존 그림에 카탈로그 역적용 — 43개 완료, 계속 하면 됩니다

**끝낸 것 (2026-08-31 · 43개)** — 전부 "의도는 이미 그 모델인데 사각형으로 그려져 있던" 것들입니다.

1~3차 15개(아래 표) + 4차 4개 + 5~8차 각 5개 + 9차 4개.
...
```bash
node guide-src/tools/chartcand.mjs            # 전체 (지금 29개)
...
| 후보 | 내용 | 바꿀 모델 |
|---|---|---|
| `csharp/t03` | Polly 전략 타임아웃 10 / 15 / 3초 | `gantt` — 겹겹이 감싸는 게 요점 |
| `js-ts/x15` | 이어받기 — 10MB 중 6MB 받고 4MB 남음 | `stack` |
| `cpp/w12` | 멀티스테이지 40 / 130 / 12MB | `bar` (server/d03 과 같은 꼴) |
| `python/t14` `python/a09` | 재시도 간격 1 / 2 / 4초 | `timeline` — js-ts/s13 를 그대로 이식 |
| `kotlin/wf01` `kotlin/wf06` | 스레드 200 vs 코루틴 · 동시 100건 | `bar` (kt07 과 같은 꼴) |

...
### 이번 회차 — 카탈로그 역적용 4·5차 (그림 9개 · §3 에 상세)

새로 그린 그림은 없습니다. **이미 있던 그림 9개를 카탈로그 모델로 다시 그렸습니다.**
그중 4개는 **막대가 값에 비례하지 않던** 것이었습니다 —
`java/a07`(200·300·250ms 가 전부 같은 폭) · `cpp/x06`(용량 1→32 가 전부 같은 폭) ·
`rust/j09`(첫 막대만 축척이 다름) · `cpp/h10`·`cpp/h09`(수치가 글로만 있었음).
...
### 3. 🟡 기존 그림에 카탈로그 역적용 — 24개 완료, 계속 하면 됩니다

**끝낸 것 (2026-08-31 · 24개)** — 전부 "의도는 이미 그 모델인데 사각형으로 그려져 있던" 것들입니다.

1~3차 15개(아래 표) + 4차 4개 + 5차 5개.
...
```bash
node guide-src/tools/chartcand.mjs            # 전체 (지금 48개)
...
| 후보 | 내용 | 바꿀 모델 |
|---|---|---|
| `cpp/h07` | 영상 디코딩 4단계가 전체 시간의 78/12/8/2% | `stack` (100% 누적 한 줄) |
| `kotlin/sc11` | 고정 윈도우 경계 문제 (60·120초 경계에 몰림) | `timeline` 또는 `bar` |
| `csharp/z06` | 스레드 풀 기아 (요청 1000 vs 스레드 16) | `bar` — 배율이 안 보임 |
| `java/z06` | G1 리전별 쓰레기 비율 12·88·91·35·76% | `matrix` 순차 램프(`.h0~.h5`) |
| `server/m06` | 동기 호출 사슬 100+50+800 = 950ms | 팬아웃 그림은 두고 `waterfall` 한 줄 덧대기 |

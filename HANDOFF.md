...
### 이번 회차 — 카탈로그 역적용 4~7차 (그림 19개 · §3 에 상세)

새로 그린 그림은 없습니다. **이미 있던 그림 19개를 카탈로그 모델로 다시 그렸습니다.**
...
### 3. 🟡 기존 그림에 카탈로그 역적용 — 34개 완료, 계속 하면 됩니다

**끝낸 것 (2026-08-31 · 34개)** — 전부 "의도는 이미 그 모델인데 사각형으로 그려져 있던" 것들입니다.

1~3차 15개(아래 표) + 4차 4개 + 5차 5개 + 6차 5개 + 7차 5개.
...
```bash
node guide-src/tools/chartcand.mjs            # 전체 (지금 38개)
...
| 후보 | 내용 | 바꿀 모델 |
|---|---|---|
| `db/a08` | N+1 — 2ms 짜리 쿼리 101번 vs 4ms 한 번 | `bar` — 왕복 횟수가 곧 길이 |
| `kotlin/kt07` `kotlin/wf06` | 코루틴 1000개가 커넥션 10개를 나눠 씀 | `bar` — 1000 대 10 의 배율 |
| `server/d03` | 레이어 캐시 180초 → 40초 → 0초 | `bar` 또는 `waterfall` |
| `java/f15` | 워커 4개 중 1개가 막히면 트래픽 25% 인질 | `stack` — 25% 를 길이로 |
| `cpp/n02` | 고정 틱 33.3ms 중 일하는 시간과 남는 시간 | `stack` — 예산 대비 사용률 |

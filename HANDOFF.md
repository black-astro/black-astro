...
| (이번) | CS 섹션 3개(`l15` SVD·최소제곱 · `s15` 스킵리스트·지속자료구조 · `o14` 스케줄러 내부) + 시각화 12개 (26 → 38) |
...
### 다이어그램 현황 (`.diag` 인라인 SVG) — 총 164개
| 가이드 | 개수 | 실측 |
|---|---|---|
| **cs** | **38** | ✅ 넘침·겹침 0 (12탭 전부 3개 이상) |
| csharp | 32 | ✅ |
| db | 27 | ✅ |
| python | 23 | ✅ |
| js-ts | 22 | ✅ |
...
CS 탭별 diag: math 2 · la 7 · comp 2 · ds 4 · algo 2 · arch 2 · os 4 · net 3 ·
lang 3 · sec 3 · dist 3 · se 3 — **“탭당 1개뿐”이던 곳(09·10·12)은 해소됐습니다.**
...
현재 **165 섹션**. 지금까지 추가한 것: `a14` 분할상환 · `a15` 네트워크 플로우 ·
...
> **주의** — `d04` 락프리·CAS, `d11` 분산 트랜잭션·사가는 **이미 있습니다.**
> HANDOFF 의 옛 후보 목록(“락프리 자료구조”, “2PC vs Saga”)은 중복이었습니다.
> 후보를 고르기 전에 `11-sidebar.html` 의 해당 `navset` 목차를 먼저 읽으세요.

### 2. 시각화 계속
**CS 는 12탭 전부 3개 이상을 채웠습니다.** 다음 목표를 잡는다면
`01-math`(2) · `03-comp`(2) · `05-algo`(2) · `06-arch`(2) 를 3개로 올리는 쪽입니다.

...
### 이번 회차 — 카탈로그 역적용 4·5·6차 (그림 14개 · §3 에 상세)

새로 그린 그림은 없습니다. **이미 있던 그림 14개를 카탈로그 모델로 다시 그렸습니다.**
...
### 3. 🟡 기존 그림에 카탈로그 역적용 — 29개 완료, 계속 하면 됩니다

**끝낸 것 (2026-08-31 · 29개)** — 전부 "의도는 이미 그 모델인데 사각형으로 그려져 있던" 것들입니다.

1~3차 15개(아래 표) + 4차 4개 + 5차 5개 + 6차 5개.
...
```bash
node guide-src/tools/chartcand.mjs            # 전체 (지금 43개)
...
| 후보 | 내용 | 바꿀 모델 |
|---|---|---|
| `js-ts/v08` `js-ts/f11` | 지연 예산 150ms 를 층별로 쪼갠 것 | `waterfall` — 예산선 하나 그으면 끝 |
| `java/g12` | 카나리 5% → 25% → 50% → 100% | `stack` 또는 `gantt` (단계별 노출 비율) |
| `cs/m06` | PCA 축별 설명 분산 52·24·11% | `bar` — 스크리 그래프가 정석입니다 |
| `server/d06` | 도커만으로 안 되는 일 (2·1·3·20) | 수치가 종류라 차트가 아닐 수 있음 — 원본 확인 후 판단 |

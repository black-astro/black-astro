# 핸드오프 — 언어 가이드 확장 (2026-08-31 15차 갱신)
...
### 3. 🟡 기존 그림에 카탈로그 역적용 — 1차 7개 완료, 계속 하면 됩니다

**끝낸 것 (2026-08-31 · 7개)** — 전부 "의도는 이미 그 모델인데 사각형으로 그려져 있던" 것들입니다.
...
```bash
node guide-src/tools/chartcand.mjs            # 전체 (지금 64개)
...
| 후보 | 내용 | 바꿀 모델 |
|---|---|---|
| `js-ts/g07` `js-ts/v14` | 콜드 스타트 · 앱 기동 시간 분해 | `waterfall` |
| `server/m10` | 분산 트레이스 (y07 과 같은 형태) | `waterfall` |
| `server/y04` | 히스토그램 버킷에서 p95 뽑기 | `bar` |
| `csharp/b10` | WASM 게시 크기 | `bar` |
| `cpp/w06` `rust/u01` | 처리량 · 배포물 크기 | `bar` |
| `python/g01` | 입력이 10배면 시간은 몇 배 | `line` |

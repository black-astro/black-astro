# 핸드오프 — 언어 가이드 확장 (2026-08-28 14차 갱신)
...
### 3. 🔵 다음 후보 — 기존 그림에 카탈로그 역적용

**가장 값이 큰 다음 과제일 것입니다.** 카탈로그(30종)는 만들었지만,
기존 그림 1,340여 개는 여전히 **96%가 "사각 박스 + 직선"** 입니다.
C# 3탭에서 확인했듯 모델을 바꾸면 같은 내용이 훨씬 빨리 읽힙니다.

**고르는 법** — 전부 바꿀 필요는 없습니다. 아래에 해당하는 것만 찾습니다.

| 지금 그린 모양 | 내용이 실은 | 바꿀 모델 |
|---|---|---|
| 박스 나열 + 숫자 라벨 | 값의 크기 비교 | `bar` |
| 박스 나열 + 시간 표기 | 시간에 따른 변화 | `line` · `waterfall` |
| 좌우 화살표 왕복 3회 이상 | 프로토콜 · 인증 왕복 | `seq` |
| 박스 옆에 "누가" 가 적혀 있음 | 역할별 분담 | `swim` |
| "A 를 쓸까 B 를 쓸까" 표 | 선택 기준 | `decision` · `quad` |
| 단계마다 건수가 줄어듦 | 전환 · 필터 | `funnel` |
| 비율·구성비를 글로 적음 | 구성비 | `stack` |

**찾는 방법** — 그림 안에 숫자와 단위(`ms` `MB` `%` `건`)가 있는데 `rect` 만 있는 경우가 1순위입니다.
```bash
node -e "const fs=require('fs'),p=require('path');for(const g of ['cpp','server']){const d='guide-src/'+g+'/parts/panes';for(const f of fs.readdirSync(d)){const s=fs.readFileSync(p.join(d,f),'utf8');for(const m of s.matchAll(/<svg viewBox[\s\S]*?<\/svg>/g)){const v=m[0];if(/\d+\s*(ms|MB|GB|%|건|초)/.test(v)&&!/class=\"(bar|li|area|band|cel)/.test(v))console.log(g,f)}}}"
```

한 번에 다 하지 말고 **탭 하나씩** 훑으면서 명백한 것만 바꾸세요.
바꾼 뒤에는 `svgcheck` → 브라우저 실측(§1 절차)을 반드시 다시 돌립니다.

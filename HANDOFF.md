...
### 이번 회차 — 카탈로그 역적용 4~8차 (그림 24개 · §3 에 상세)

새로 그린 그림은 없습니다. **이미 있던 그림 24개를 카탈로그 모델로 다시 그렸습니다.**
...
### 3. 🟡 기존 그림에 카탈로그 역적용 — 39개 완료, 계속 하면 됩니다

**끝낸 것 (2026-08-31 · 39개)** — 전부 "의도는 이미 그 모델인데 사각형으로 그려져 있던" 것들입니다.

1~3차 15개(아래 표) + 4차 4개 + 5~8차 각 5개.
...
```bash
node guide-src/tools/chartcand.mjs            # 전체 (지금 33개)
...
| 후보 | 내용 | 바꿀 모델 |
|---|---|---|
| `js-ts/s13` | 큐 — 3초 접수 vs 12초·48초 처리 | `bar` 또는 `gantt` |
| `cpp/v04` `cpp/v12` | 복사 횟수별 3.1 / 0.9 / 2.2ms | `bar` |
| `js-ts/f09` | 서버리스 인스턴스 1 / 10 / 100 개 | `bar` — 커넥션 폭증의 배율 |
| `cs/l14` | 엔트로피 H — 확률 50/90/100% 별 질문 횟수 | `line` (곡선이 요점) |
| `server/o09` | 무중단 배포 세 방식의 자원 5 / 25 / 100% | `bar` + `compare` |

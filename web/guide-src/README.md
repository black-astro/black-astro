# 학습 가이드 소스 (Python · Java · Kotlin · JS/TS · C#/Unity · C++ · Rust · Database · 서버기술 · CS 기술)

`public/*-web/index.html` 열 개의 **원본**입니다.
...
| 가이드 | 소스 | 배포 경로 | 규모 |
|---|---|---|---|
| 🐍 Python | `guide-src/python/` | `/python-web/` | 17탭 · 256섹션 |
| ☕ Java | `guide-src/java/` | `/java-web/` | 14탭 · 221섹션 |
| 🟠 Kotlin | `guide-src/kotlin/` | `/kotlin-web/` | 12탭 · 189섹션 |
| 🟨 JS · TS | `guide-src/js-ts/` | `/js-ts-web/` | 15탭 · 233섹션 |
| 🟣 C# · Unity | `guide-src/csharp/` | `/csharp-web/` | 10탭 · 141섹션 |
| 🔵 C++ | `guide-src/cpp/` | `/cpp-web/` | 8탭 · 88섹션 |
| 🦀 Rust | `guide-src/rust/` | `/rust-web/` | 8탭 · 88섹션 |
| 🗄️ Database | `guide-src/db/` | `/db-web/` | 12탭 · 179섹션 |
| 🌐 서버기술 | `guide-src/server/` | `/server-web/` | 14탭 · 186섹션 |
| 🎓 CS 기술 | `guide-src/cs/` | `/cs-web/` | 12탭 · 173섹션 |

열 가이드는 **뼈대 CSS 5개와 사이드바 스크립트를 공유**합니다(테마 동일).
공유 조각은 `guide-src/shared/` 한 곳에만 있습니다 — **한 번 고치면 열 가이드에 함께 반영됩니다.**
...
| 명령 | 하는 일 |
|---|---|
| `npm run build:guide` | 조각 → 각 `index.html` 생성 (열 가이드 모두) |
| `npm run build:guide -- java` | 특정 가이드만 (`python` · `java` · `kotlin` · `js-ts` · `csharp` · `cpp` · `rust` · `db` · `server` · `cs`) |
...
열 가이드가 같은 뼈대를 씁니다. 조각을 추가·삭제하면
**`parts.json` 도 함께 고쳐야 합니다** (누락·고아 파일은 빌드가 에러로 잡아 줍니다).

```
guide-src/
  build.mjs            빌더 (의존성 없음 · 열 가이드를 모두 처리)
  verify.mjs           결과물 정합성 · 회귀 점검
  README.md            이 문서

  shared/              ★ 열 가이드가 글자 하나까지 같이 쓰는 조각
    css/                 여기를 고치면 열 가이드가 함께 바뀝니다
  ...
  <가이드>/            python · java · kotlin · js-ts · csharp · cpp · rust · db · server · cs
...
공용 조각을 고칠 때는 **열 가이드가 모두 바뀐다는 점**을 염두에 두세요.
`npm run verify:guide` 가 열 개를 한꺼번에 점검하므로 어긋나면 바로 드러납니다.
...
| ☕ Java | Java 기초 · 고급/동시성 · Spring Boot · WebFlux · 데이터/JPA · **JPA 심화** · **QueryDSL** · **MyBatis** · Security · Gateway · 실전 도구(+JSpecify) · JavaFX · **대규모 트래픽** · **전문가** |
| 🟠 Kotlin | Kotlin 기초 · Kotlin 고급 · **코루틴/Flow** · Spring Boot · WebFlux · 데이터/JPA · **Android 기초** · **Android 실전** · **멀티플랫폼 KMP** · 대규모 트래픽 · **전문가(K2·바이트코드)** · 실전 도구 |
...
- **역할 분담**
  - 언어 가이드(🐍 ☕ 🟠 🟨 🟣) — 그 언어로 코드를 쓰는 법
    (🟠 Kotlin 은 **서버 · 안드로이드 · 멀티플랫폼**을 한 언어로 이어 붙인 형태입니다)
...
- 스타일은 각 가이드의 `css/06-*.css` 맨 아래 **`개념 그림` 블록**에 있습니다(열 가이드 동일)

...
const TAB_KW = {
  ...
  log:"로그 로그운영 log logging 로깅 액세스로그 accesslog access_log 에러로그 error_log 엔진엑스 nginx 로그포맷 log_format escape=json request_time upstream_response_time upstream_addr request_id 요청ID X-Request-Id 조건부로깅 map if= split_clients 샘플링 sampling 헬스체크 소음 아파치 apache LogFormat CustomLog rotatelogs 톰캣 tomcat AccessLogValve catalina.out 캐디 caddy 시스로그 syslog rsyslog 저널 journald journalctl SystemMaxUse RateLimitBurst 로그로테이트 logrotate copytruncate USR1 delaycompress inode 수집기 collector 플루언트비트 FluentBit fluent-bit 벡터 Vector VRL 파일비트 Filebeat 플루언트디 Fluentd 프롬테일 Promtail Alloy 로키 Loki LogQL 엘라스틱 Elasticsearch OpenSearch ILM 클릭하우스 ClickHouse 클라우드로그 CloudWatch 저장소 보존 retention 비용 압축 gzip zstd 마스킹 masking 개인정보 접속기록 감사로그 audit auditd ausearch chattr ObjectLock 접근통제 권한 쿠버네티스 kubernetes k8s kubectl logs containerd CRI kubelet containerLogMaxSize DaemonSet 사이드카 ingress-nginx 장애조사 트러블슈팅 troubleshooting jq 5xx 502 504 499 디스크가득 truncate lsof deleted",
};
...
const SEC_KW = {
  ...
  lg01:"로그지도 원천 앱 웹서버 OS 커널 stdout access_log error_log journald dmesg 디스크가득 df du lsof deleted truncate SystemMaxUse 상한 파티션",
  lg02:"nginx log_format access_log combined request_time upstream_response_time upstream_header_time upstream_connect_time upstream_addr request_id time_iso8601 buffer flush 워터폴 변수",
  lg03:"escape=json JSON로그 error_log 레벨 log_not_found map if= split_clients 샘플링 헬스체크 정적파일 access_log off 소음 조건부로깅 upstream timed out connect failed",
  lg04:"apache LogFormat CustomLog ErrorLogFormat %D %L rotatelogs 파이프로그 env= tomcat AccessLogValve JsonAccessLogValve catalina.out JULI caddy Caddyfile roll_size filter 매트릭스",
  lg05:"syslog facility severity 시설 심각도 local7 journald journalctl --since -p err -b -1 SystemMaxUse RateLimitBurst Storage=persistent rsyslog omfwd imfile RELP 원격전송 StandardOutput",
  lg06:"logrotate inode rename USR1 postrotate delaycompress sharedscripts copytruncate dateext compresscmd zstd 재오픈 회전 logrotate.timer -d 예행연습 수집기 Refresh_Interval",
  lg07:"수집기 FluentBit fluent-bit Vector VRL Filebeat Fluentd Promtail Alloy 히트맵 비교 tail storage.type filesystem Mem_Buf_Limit DB 오프셋 디스크버퍼 remap sample vector validate vrl tap 취합기",
  lg08:"Loki Elasticsearch OpenSearch ClickHouse 클라우드로그 CloudWatch 색인 라벨 역색인 칼럼 retention_period ILM hot warm cold 스트림 카디널리티 structured metadata 수집요금 ingest S3 아카이브 하이브리드",
  lg09:"X-Request-Id request_id map proxy_set_header add_header always 요청ID 전파 HAProxy unique-id Apache UNIQUE_ID Caddy http.request.uuid traceparent LogQL 시퀀스 SSI 오류페이지",
  lg10:"비용 용량계산 43GB 압축 gzip zstd 샘플링 sample_rate 가중치 보존 retention 뜨거움 미지근 차가움 DEBUG끄기 UserAgent Referer 축약 발산램프 트레이드오프 막대",
  lg11:"마스킹 masking 개인정보 토큰 Authorization Cookie Referer IP마스킹 map REDACTED client_body_in_file_only 접속기록 보관 안전성확보조치 감사로그 audit auditd ausearch aureport log-tamper chattr +a ObjectLock COMPLIANCE 접근권한 0640 adm systemd-journal 자격증명 분리 스윔레인",
  lg12:"쿠버네티스 kubernetes k8s kubectl logs --previous containerd CRI /var/log/pods /var/log/containers kubelet containerLogMaxSize containerLogMaxFiles docker json-file max-size DaemonSet FluentBit kubernetes필터 Merge_Log fluentbit.io/exclude ingress-nginx log-format-upstream 사이드카 장애조사 깔때기 jq LogQL topk 502 504 499 upstream 트러블슈팅",
};

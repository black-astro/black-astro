...
const TAB_KW = {
  ...
  next:"next nextjs next.js 넥스트 넥스트js 넥스트제이에스 앱라우터 approuter app-router pagesrouter 라우터 router 서버컴포넌트 rsc reactservercomponents useclient useserver 클라이언트컴포넌트 경계 boundary ssr ssg isr ppr csr 서버렌더링 정적생성 증분재생성 부분사전렌더 프리렌더 prerender 하이드레이션 hydration 스트리밍 streaming suspense loading error notfound layout template page route routehandler 라우트핸들러 미들웨어 middleware proxy matcher edge 엣지 런타임 runtime nodejs serveraction serveractions 서버액션 useactionstate useformstatus useoptimistic usetransition formaction 캐시 cache 캐싱 caching revalidate revalidatetag revalidatepath unstable_cache usecache cachelife cachetag datacache fullroutecache routercache 요청메모이제이션 memoization fetch nostore forcecache tags 태그 무효화 generatestaticparams dynamicparams dynamic forcedynamic forcestatic generatemetadata metadata sitemap robots opengraph og imageresponse nextog 공유카드 seo 검색노출 크롤러 lighthouse corewebvitals lcp inp cls 번들 bundle bundleanalyzer 번들분석 dynamicimport 코드분할 codesplitting lazy authjs nextauth auth.js nextauthv5 인증 로그인 세션 session jwt oauth github credentials clerk supabase betterauth prisma drizzle orm 마이그레이션 migration 커넥션풀 connectionpool pgbouncer supavisor neon serverlessdriver postgres postgresql sql n+1 엔플러스원 트랜잭션 transaction tailwind tailwind4 shadcn shadcnui radix cssmodules 다크모드 darkmode nextthemes nextfont nextimage 폰트 이미지 avif webp sharp priority sizes placeholder blur 배포 deploy vercel 버셀 도커 docker standalone dockerfile nginx 리버스프록시 reverseproxy 자체호스팅 selfhosting 환경변수 env nextpublic 빌드캐시 buildcache turbopack instrumentation 관측 observability 로그 logging pino sentry params searchparams cookies headers redirect notfound usepathname userouter usesearchparams link prefetch 병렬라우트 parallelroutes 인터셉트 interceptingroutes 라우트그룹 routegroup 동적세그먼트 catchall 블로그 관리자 admin crud zod 검증 validation 폼 form",
  edge:"hono 호노 혼오 edge 엣지 에지 엣지런타임 edgeruntime 경량 경량서버 웹표준 webstandard fetch fetchapi request response 리퀘스트 리스폰스 표준요청 이식성 portable 런타임 runtime bun 번 번런타임 bunjs bunsh oven javascriptcore jsc bunserve bunfile bunsqlite bunpassword bunshell bunx buninstall buntest bunbuild bunhot compile 단일실행파일 deno 데노 데노2 deno2 denodeploy denokv denoserve denocompile 권한 permission allownet allowread allowwrite allowenv 샌드박스 sandbox jsr denojson denotask npm지정자 npmspecifier cloudflare 클라우드플레어 workers 워커스 cloudflareworkers wrangler 랭글러 workerd 아이솔레이트 isolate v8isolate 콜드스타트 coldstart 서버리스 serverless faas 바인딩 binding kv kvnamespace d1 r2 durableobjects durableobject do 내구성객체 queues 큐 cron crontrigger smartplacement 스마트배치 hyperdrive 하이퍼드라이브 nodejscompat compatibilitydate vercel 버셀 verceledge netlify fastly deployctl 미들웨어 middleware 양파 onion awaitnext cset cget cenv cvar variables bindings 제네릭 generic createmiddleware 팩토리 factory httpexception onerror notfound secureheaders cors compress logger bearerauth jwt basicauth etag requestid timing prettyjson csrf cache zod 검증 validation zvalidator honozod openapi zodopenapi scalar swagger 문서자동생성 rpc hc honoclient inferrequesttype inferresponsetype 타입공유 typesharing 체이닝 chaining trpc 비교 drizzle 드리즐 drizzleorm drizzlekit prisma prismaaccelerate accelerate 드라이버어댑터 driveradapter neon 네온 turso 투르소 libsql planetscale supabase postgres postgresql sqlite 커넥션풀 connectionpool 커넥션 pgbouncer supavisor 트랜잭션 transaction batch 조건부업데이트 마이그레이션 migration 캐시 cacheapi cachescontrol swr 데이터지역성 datalocality 리전 region colo 스트리밍 streaming readablestream transformstream sse eventsource streamsse 서버전송이벤트 websocket 웹소켓 upgradewebsocket hibernation llm 토큰스트리밍 tokenstreaming 역압 backpressure 테스트 test apprequest testclient vitest vitestpoolworkers miniflare supertest 관측 observability wranglertail logs logpush tailworker 로그 opentelemetry otel traceparent 트레이스 sentry toucan 요청id requestid 배포 deploy 롤백 rollback versions 점진배포 canary 시크릿 secret devvars 환경변수 env 커스텀도메인 customdomain 도커 docker 멀티스테이지 multistage 비용 요금 pricing 요청당과금 cpu시간 url단축 urlshortener 단축주소 클릭통계 리다이렉트 redirect waituntil executioncontext webcrypto pbkdf2 bcrypt대체 express비교 expressvs 이식",
  log:"로깅 로그 log logging 로거 logger console console.log console.error stdout stderr 파일서술자 fd 동기쓰기 블로킹 이벤트루프 event loop util.inspect pino 피노 winston 윈스턴 bunyan 버니언 roarr debug consola tslog signale 라이브러리비교 채택 npm trends 다운로드 pino-http pino-pretty pino-roll nestjs-pino 트랜스포트 transport 워커스레드 worker thread sonic-boom 자식로거 child logger base level redact 레닥트 마스킹 민감정보 토큰 비밀번호 password authorization 쿠키 cookie 카드번호 JWT 유출 winston-daily-rotate-file logrotate 회전 rotation combine format errors stack json printf 구조적로깅 structured logging JSON 필드 field 검색 쿼리 ECS OpenTelemetry OTel semantic conventions 로그레벨 level fatal error warn info debug trace silent 샘플링 sampling 표본 필드사전 Express 익스프레스 Fastify 패스티파이 NestJS 네스트 Next.js 넥스트 morgan 모건 접근로그 access log 미들웨어 middleware server-only Edge runtime 런타임 AsyncLocalStorage ALS 비동기컨텍스트 요청ID reqId requestId x-request-id traceparent traceId spanId 상관관계 correlation mixin OpenTelemetry TypeScript 타입 리터럴유니온 판별유니온 초과속성검사 브랜드타입 Record any 타입안전 성능 p99 루프지연 event loop delay 백프레셔 backpressure 유실 drop sync 비동기 minLength flush SIGTERM 종료 graceful 브라우저 browser 링버퍼 breadcrumb 발자국 소스맵 sourcemap Script error crossorigin Sentry 센트리 RUM 비콘 beacon sendBeacon 클라이언트로그 log injection 로그위조 수집기 collector Vector 벡터 Fluent Bit 플루언트비트 Promtail Alloy Loki 로키 Elasticsearch OpenSearch ClickHouse Datadog CloudWatch 12-factor 이벤트스트림 라벨 label 카디널리티 매핑폭발 알림 alert 비율 SLO 비용 보존 retention",
};
...
const SEC_KW = {
  ...
  f01:"csr spa seo 검색노출 첫화면 ssg ssr isr ppr 렌더링전략 비교 선택흐름도 astro reactrouter7 remix vite createnextapp pagesrouter 마이그레이션 getserversideprops",
  f02:"approuter app폴더 page layout template loading error notfound route default 라우트그룹 group 동적세그먼트 catchall slug 병렬라우트 slot 인터셉트 모달 params await link userouter navigation",
  f03:"rsc 서버컴포넌트 클라이언트컴포넌트 useclient 경계 번들 직렬화 props 함수전달금지 children 슬롯 serveronly clientonly provider hydration window dynamic ssrfalse queryclient 전역변수",
  f04:"fetch await 캐시 nostore forcecache revalidate tags revalidatetag revalidatepath unstable_cache usecache 요청메모이제이션 reactcache 데이터캐시 라우트캐시 라우터캐시 워터폴 promiseall next15 로그",
  f05:"serveraction useserver formaction useactionstate useformstatus useoptimistic usetransition zod 검증 에러반환 redirect try catch bind csrf 권한 인가 보안 revalidate 낙관적업데이트",
  f06:"generatestaticparams dynamicparams revalidate dynamic forcedynamic forcestatic cookies headers connection suspense loading 스트리밍 ppr cachecomponents 빌드출력표 static dynamic 기호 스켈레톤 cls",
  f07:"routehandler route.ts get post request response nextresponse 웹훅 webhook 서명검증 stripe csv 스트림 미들웨어 middleware matcher redirect rewrite next 엣지 edge cors ratelimit upstash sitemap robots proxy",
  f08:"authjs nextauth v5 로그인 세션 jwt database strategy providers github credentials callbacks authorized signin signout middleware 보호 role 권한 clerk supabase betterauth authsecret bcrypt",
  f09:"prisma drizzle orm schema 마이그레이션 migrate deploy studio 커넥션풀 connectionlimit pgbouncer supavisor directurl neon serverless 드라이버 어댑터 n+1 include reactcache 트랜잭션 무중단 스키마변경",
  f10:"tailwind tailwind4 theme oklch shadcn radix cssmodules clsx twmerge nextfont localfont 한글폰트 서브셋 nextimage sizes priority fill placeholder avif webp remotepatterns sharp 다크모드 nextthemes cssinjs",
  f11:"성능 번들분석 bundleanalyzer analyze dynamicimport 코드분할 generatemetadata metadatabase canonical sitemap robots opengraph imageresponse nextog jsonld 구조화데이터 lcp inp cls corewebvitals lighthouse webvitals rum",
  f12:"배포 vercel 도커 docker dockerfile standalone multistage nginx 리버스프록시 proxybuffering xforwardedproto 환경변수 nextpublic buildarg zod env 검증 cachehandler isr공유 빌드캐시 githubactions instrumentation 헬스체크 pino sentry",
  f13:"실전 프로젝트 블로그 관리자 admin crud 폴더구조 features drizzle schema queries actions zod 폼 postform isr revalidatetag generatestaticparams generatemetadata authjs 미들웨어 docker 배포 체크리스트 로드맵",
  g01:"웹표준 request response 이식성 노드전용 nodehttp 런타임지형도 컨테이너 아이솔레이트 콜드스타드 콜드스타트 서버리스 요청당과금 인스턴스과금 선택흐름도 workers vercel denodeploy 비용모델",
  g02:"설치 createhono 첫서버 creq cjson ctext chtml 파라미터 param query parsebody 라우팅 route basepath 라우터분리 어댑터 nodeserver denoserve bunserve appfetch express대조 대응표 return404",
  g03:"미들웨어 양파 awaitnext appuse 등록순서 경로범위 내장미들웨어 logger cors securehealders securehearders bearerauth jwt cache etag requestid cset cget bindings variables 제네릭 createmiddleware httpexception onerror notfound 전역변수금지 지연로딩",
  g04:"zod 검증 zvalidator creqvalid hook 에러형태 masssignment massassignment rpc hc appType 체이닝 inferrequesttype inferresponsetype url 모노레포 타입공유 openapi openapihono createroute scalar securityschemes trpc비교 추론속도",
  g05:"bun 설치 bunrun buninstall buntest bunbuild bunhot compile 단일실행파일 javascriptcore bunserve bunfile bunwrite bunsqlite bunpassword bunshell bunenv worker bunsql 노드호환 nodeapi 네이티브애드온 apm 관측미지원 도입순서 autocannon setupbun",
  g06:"deno deno2 설치 권한 allownet allowread allowwrite allowenv denywrite 샌드박스 denojson imports tasks npm지정자 jsr denoinstall denoadd fmt lint check test bench doc coverage compile denokv atomic sum enqueue listenqueue denoserve servestatic 비교표",
  g07:"cloudflare workers wrangler wranglerjsonc compatibilitydate nodejscompat wranglerdev remote deploy tail secretput 아이솔레이트 v8 컨테이너비교 콜드스타트 바인딩 kv d1 r2 durableobjects queues service cenv waituntil executioncontext scheduled crontrigger wranglertypes vitestpoolworkers miniflare",
  g08:"cpu시간 실행시간상한 메모리128mb 파일시스템없음 r2 사전서명url 정적자산 커넥션풀 toomanyconnections httpdriver hyperdrive pgbouncer supavisor rdsproxy smartplacement 데이터지역성 지역분산 읽기복제본 번들크기 서브리퀘스트 지연로딩",
  g09:"drizzle drizzlekit schema relations infer 마이그레이션 generate migrate studio d1 batch neon neonhttp turso libsql supabase prisma accelerate driveradapter 대화형트랜잭션없음 조건부업데이트 낙관적잠금 cacheapi kv 캐시무효화 selectstar n+1 앞뒤호환",
  g10:"readablestream transformstream 스트리밍 streamsse writesse onabort sse eventsource lasteventid llm 토큰 중계 pipethrough 브라우저reader websocket upgradewebsocket bunserve denoupgrade durableobjects websocketpair hibernation acceptwebsocket 역압 backpressure abortsignal notransform r2스트리밍",
  g11:"apprequest testclient vitest buntest denotest supertest비교 cloudflaretest env self vitestpoolworkers miniflare 구조화로그 json console cfray colo country wranglertail workerslogs 샘플링 logpush tailworker opentelemetry otel traceparent sentry toucan waituntil onerror servertiming",
  g12:"deploy wranglerdeploy versionsupload versionsdeploy rollback deployments 환경분리 env staging routes customdomain 인증서 deployctl denodeploy vercel handle 도커 dockerfile 멀티스테이지 ovenbun distroless healthcheck 시크릿 secret devvars gitignore rotation githubactions 요금 요청당 cpu시간 egress 체크리스트",
  g13:"url단축 urlshortener 클릭통계 리다이렉트 302 폴더구조 wranglerjsonc drizzle schema links events jwt honojwt webcrypto pbkdf2 zvalidator 체이닝 apptype hc kv캐시 waituntil batch 집계 groupby cron 테스트 self env 배포 비교표 콜드스타트 응답시간",
  lg01:"console console.log console.warn console.error console.debug stdout stderr 파일서술자 fd 1 2 리다이렉트 2>&1 동기 synchronous 블로킹 이벤트루프 event loop 파이프 pipe TTY util.inspect 지연평가 객체참조 스냅샷 breadcrumb 발자국 검색 grep 일회성 CLI 사용자출력 기준",
  lg02:"비교 라이브러리 pino 피노 winston 윈스턴 bunyan roarr debug consola tslog signale 히트맵 JSON계열 콘솔계열 채택 npm trends 다운로드수 유지보수 Fastify 기본 pino-http nestjs-pino DEBUG 환경변수 pino-pretty 결정표 신규채택 멈춘프로젝트",
  lg03:"pino logger.info 인자순서 객체먼저 메시지 문자열조립 JSON.stringify 사전컴파일 level base redact child 자식로거 문맥 고정필드 transport 트랜스포트 pino-pretty 개발 운영 워커스레드 worker sonic-boom isLevelEnabled 지연평가 expensive err 키 스택 serializers sync flush AsyncLocalStorage",
  lg04:"winston format combine errors stack json printf timestamp 순서 transports Console File Http 목적지 레벨 npm levels error warn info http verbose debug silly 반대 pino 비교 child 자식로거 winston-daily-rotate-file 회전 logrotate 컨테이너 원격트랜스포트 마이그레이션 얇은인터페이스 병행",
  lg05:"구조적로깅 structured logging JSON 필드 field 검색 필터 문장 메시지 고정문구 ECS @elastic/ecs-pino-format OpenTelemetry semantic conventions time @timestamp level log.level msg message trace.id 일관성 매핑폭발 mapping explosion 필드사전 dictionary 코드로관리 레벨설계 fatal error warn info debug trace 샘플링 sampled 단위 durationMs sizeBytes",
  lg06:"Express 익스프레스 Fastify 패스티파이 NestJS 네스트 Next.js 넥스트 pino-http nestjs-pino morgan 모건 접근로그 미들웨어 middleware 요청진입 요청스코프 응답완료 onResponse hook logger:true bufferLogs Logger 중복로그 server-only Edge runtime Node runtime 클라이언트컴포넌트 worker_threads 형식맞추기 console",
  lg07:"AsyncLocalStorage ALS 비동기컨텍스트 async_hooks run getStore 요청ID reqId requestId 전파 propagation mixin child 콜백 서버시작 문맥유실 x-request-id traceparent W3C OpenTelemetry trace.getActiveSpan traceId spanId 상관관계 correlation 오류코드 사용자노출 지연측정 응답완료 한곳",
  lg08:"TypeScript 타입 리터럴유니온 literal union 판별유니온 discriminated union 초과속성검사 excess property check 인터페이스 interface Record any 브랜드타입 branded type 오타 필수필드 컴파일오류 런타임 as 타입주장 unknown 제네릭 로그스키마 이벤트이름 event",
  lg09:"성능 performance 이벤트루프 event loop 루프지연 delay p99 동기 sync 비동기 async 블로킹 파이프 stdout 파일 sonic-boom minLength 버퍼 백프레셔 backpressure 유실 drop mode=non-blocking flush SIGTERM kill -9 종료 graceful shutdown 감사로그 결제 best-effort LOG_LEVEL silent 벤치마크 측정",
  lg10:"민감정보 PII redact 마스킹 masking 토큰 token 비밀번호 password authorization Bearer 쿠키 cookie 세션 카드번호 주민번호 JWT eyJ 유출 leak 통째로넘기기 req res err.config headers process.env sql params 쿼리스트링 originalUrl 정규식 성능 수집기 Vector Fluent Bit 무효화 rotate 사고대응 절차 Slack",
  lg11:"브라우저 browser 클라이언트 client 링버퍼 ring buffer breadcrumb 발자국 비콘 beacon sendBeacon fetch keepalive 엔드포인트 /api/client-logs 신뢰하지않는입력 로그위조 log injection 크기제한 rate limit 소스맵 sourcemap Script error crossorigin CORS release 버전 Sentry RUM OpenTelemetry 브라우저 tracePropagationTargets sentry-trace 세션리플레이 마스킹 개인정보",
  lg12:"수집기 collector 에이전트 agent Vector 벡터 Fluent Bit 플루언트비트 Promtail Alloy Fluentd 저장소 Loki 로키 LogQL Elasticsearch OpenSearch ClickHouse Datadog CloudWatch 12-factor 이벤트스트림 stdout 파이프 라벨 label 인덱스 카디널리티 매핑폭발 OpenTelemetry OTLP pino-opentelemetry-transport trace_id Tempo 알림 alert 비율 SLO 소음 비용 볼륨 GB 보존 retention 샘플링",
};
let FIDX = null;

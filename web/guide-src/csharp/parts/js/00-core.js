...
const TAB_KW = {
  ...
  log:"로깅 로그 log logging 로거 logger ILogger ILoggerFactory ILoggerProvider Microsoft.Extensions.Logging MEL 로그레벨 loglevel Trace Debug Information Warning Error Critical None 카테고리 필터 AddFilter appsettings 구조적로깅 structured 메시지템플릿 template LoggerMessage 소스제너레이터 sourcegenerator 무할당 박싱 Serilog 세리로그 시리로그 NLog 엔로그 log4net 로그포넷 ZLogger 지로거 sink 싱크 enricher Seq Elastic Elasticsearch Splunk Datadog ApplicationInsights Loki 파일로그 롤링 rolling 보관 retention 로테이션 logrotate EventLog 이벤트로그 journald stdout 콘솔 JsonConsole HttpLogging W3CLogger BeginScope 스코프 TraceId SpanId traceparent Activity ActivitySource 분산추적 distributedtracing OpenTelemetry OTLP 관측성 observability 민감정보 개인정보 마스킹 masking Destructure redaction 커넥션문자열 유출 IsEnabled 성능 할당 GC BenchmarkDotNet MemoryDiagnoser dotnet-counters dotnet-trace CloseAndFlush 유실 비동기sink 로그비용 로그검색 로그수집기 로그분석",
};
...
const SEC_KW = {
  ...
  lg01:"ILogger ILoggerFactory ILoggerProvider DI 의존성주입 카테고리 NullLogger Abstractions LoggerFactory.Create ClearProviders 추상화 라이브러리 테스트 가짜로거 널오브젝트",
  lg02:"LogLevel Trace Debug Information Warning Error Critical None appsettings Logging LogLevel Default 접두사 AddFilter SetMinimumLevel LoggingLevelSwitch reloadOnChange 환경변수 Development Production",
  lg03:"Serilog NLog log4net ZLogger 내장provider 비교 선택 채택 NuGet 다운로드 GitHub star 히트맵 sink 생태계 성능 구조적로깅 설정편의 어댑터 마이그레이션 중복로그",
  lg04:"구조적로깅 structured 메시지템플릿 messagetemplate 보간문자열 문자열결합 destructure 포맷지정자 필드이름 검색 집계 Seq쿼리 CA2017 분석기 로그뷰타입 익명타입",
  lg05:"LoggerMessage 소스제너레이터 sourcegenerator partial EventId 박싱 boxing 할당 allocation 무할당 고성능 핫패스 BenchmarkDotNet MemoryDiagnoser IsEnabled 조기반환 ZLogger",
  lg06:"Serilog UseSerilog ReadFrom.Configuration WriteTo sink Console File Seq Elasticsearch enricher FromLogContext PushProperty LogContext Expressions ByExcluding ExpressionTemplate CloseAndFlush CreateBootstrapLogger rollingInterval retainedFileCountLimit",
  lg07:"NLog nlog.config target rules autoReload final JsonLayout ColoredConsole UseNLog LogManager.Shutdown CopyToOutputDirectory throwConfigExceptions 결정흐름도 선택기준 Serilog비교 XML설정",
  lg08:"ASP.NET Core 요청로그 HttpLogging UseHttpLogging HttpLoggingFields W3CLogger UseW3CLogging BeginScope 스코프 IncludeScopes SimpleConsole JsonConsole UseSerilogRequestLogging 헬스체크 소음 TraceIdentifier 미들웨어",
  lg09:"Activity ActivitySource TraceId SpanId traceparent W3CTraceContext 분산추적 distributedtracing OpenTelemetry OTLP Jaeger Tempo AddSource AddOtlpExporter ActivityTrackingOptions Baggage 지표 Meter 관측성",
  lg10:"성능 비용 할당 allocation GC IsEnabled 조기반환 비동기sink Serilog.Sinks.Async bufferSize blockWhenFull 유실 CloseAndFlush ShutdownTimeout dotnet-counters dotnet-trace speedscope 샘플링 보관기간 저장요금 트레이드오프",
  lg11:"민감정보 개인정보 유출 마스킹 masking Destructure ByTransforming EnableSensitiveDataLogging EFCore SQL파라미터 커넥션문자열 설정덤프 HTTP본문 PersonalData SensitiveData Redaction ErasingRedactor 비밀회전 감사 SIEM 백업",
  lg12:"배포 컨테이너 쿠버네티스 stdout JsonConsole 윈도우서비스 EventLog New-EventLog IIS web.config systemd journald journalctl WPF LOCALAPPDATA 로테이션 logrotate rollingInterval fileSizeLimitBytes retainedFileCountLimit SelfLog 진단",
};

...
const TAB_KW = {
  ...
  app:"데스크톱 desktop 앱 app gui 지유아이 화면 창 window 윈도우 ui 유저인터페이스 툴킷 toolkit 위젯 widget widgets qt 큐티 qt6 qt5 pyside pyside6 pyqt 파이사이드 qml quick qtquick 선언형 declarative qmake qtcreator 큐티크리에이터 designer 디자이너 uic moc rcc automoc autouic qapplication qmainwindow qwidget qpushbutton qlabel qlineedit qtableview qtablewidget qlistview qtreeview qabstractitemmodel 모델뷰 modelview 델리게이트 delegate qsortfilterproxymodel 프록시 proxy 레이아웃 layout qvboxlayout qhboxlayout qgridlayout qformlayout splitter 시그널 signal 슬롯 slot connect 시그널슬롯 이벤트루프 eventloop exec 람다 lambda qobject 부모 parent 수명 qpointer deleteLater qthread movetothread qtconcurrent qfuture qfuturewatcher 스레드 thread 워커 worker 비동기 async qnetworkaccessmanager qnetworkreply http 네트워크 rest json qjsondocument qsettings 설정 config 레지스트리 registry savegeometry restoregeometry qsqldatabase qsqlquery qsqltablemodel sqlite 디비 db 다국어 i18n 국제화 번역 translation tr lupdate lrelease linguist qtranslator ts qm 배포 deploy windeployqt macdeployqt linuxdeployqt appimage 정적빌드 static 설치파일 installer innosetup inno nsis wix msi msix qtifw 코드서명 codesign signtool notarization 공증 smartscreen dll 디엘엘 플러그인 plugin qwindows platforms imgui dearimgui 디어이미지유아이 즉시모드 immediate 보존모드 retained glfw opengl glad 백엔드 backend imgui_demo implot imguizmo listclipper docking sdl sdl2 sdl3 에스디엘 raylib 레이립 게임루프 gameloop 델타타임 deltatime 누산기 accumulator fixedstep 스프라이트 sprite 텍스처 texture 게임패드 gamepad 오디오 audio wxwidgets wx 더블유엑스 wxframe wxsizer bind eventtable 네이티브 native 룩앤필 lookandfeel mfc win32 윈도우api winapi wndproc 메시지루프 messageloop getmessage hwnd 트레이 tray systemtray 시스템트레이 shell_notifyicon 알림 notification 토스트 toast 자동시작 autostart 시작프로그램 run키 뮤텍스 mutex 중복실행 singleinstance 파일연결 fileassociation winui winui3 windowsappsdk cppwinrt winrt xaml mica fluent juce vst3 오디오플러그인 slint 임베디드 electron 일렉트론 tauri 타우리 mvvm mvc 뷰모델 viewmodel 아키텍처 architecture 성능 performance 프로파일링 profiling tracy superluminal perf instruments 대용량 백만행 mmap 메모리매핑 디바운스 debounce 취소 cancel 로그뷰어 logviewer vcpkg cmake cmakepresets findpackage cmake_prefix_path 라이선스 license lgpl gpl 상용 commercial",
  cs:"c# 씨샵 시샵 csharp dotnet 닷넷 .net net10 net9 net8 dotnet10 clr coreclr 런타임 runtime pinvoke p/invoke 피인보크 dllimport 디엘임포트 libraryimport 라이브러리임포트 소스생성기 sourcegenerator 소스제너레이터 roslyn 로슬린 마샬링 marshalling 마샬러 marshal blittable 블리터블 structlayout sequential explicit fieldoffset pack 패딩 padding 정렬 alignment span readonlyspan memory fixed 고정 핀 pinning gchandle pinned nativememory 제로카피 zerocopy 복사없이 safehandle 세이프핸들 idisposable dispose using 핸들 handle intptr nint nuint 불투명포인터 opaque 소유권 ownership 수명 lifetime 콜백 callback 델리게이트 delegate unmanagedcallersonly delegatepointer 함수포인터 functionpointer monopinvokecallback gc 가비지컬렉터 keepalive 예외 exception 에러코드 errorcode getlastpinvokeerror setlasterror win32exception lasterror thread_local 스레드지역 c++/cli cppcli clr ref class gcnew pin_ptr marshal_as msclr 혼합어셈블리 mixed 로더락 loaderlock cppsharp swig clangsharp 바인딩 binding 자동생성 codegen 헤더 header libclang unity 유니티 il2cpp 아이엘투씨피피 mono 모노 plugins 플러그인 nativearray getunsafeptr burst jobsystem 텍스처 texture android 안드로이드 ndk arm64 ios 아이오에스 __internal 정적라이브러리 nuget 뉴겟 누겟 runtimes rid 런타임식별자 win-x64 linux-x64 osx-arm64 musl alpine 알파인 glibc setdllimportresolver nativelibrary resolver 리졸버 nativeaot aot 네이티브에이오티 publishaot directpinvoke trimming 트리밍 singlefile 단일파일 selfcontained 배포 deploy 도커 docker 컨테이너 benchmarkdotnet 벤치마크 benchmark memorydiagnoser 오버헤드 overhead aspnetcore minimalapi 미니멀에이피아이 wpf winforms 데스크톱 desktop 이미지리사이즈 resize stride 스트라이드 utf8 stringmarshalling 문자열 string dllnotfoundexception entrypointnotfoundexception badimageformatexception callingconvention cdecl stdcall extern c dllexport 공유라이브러리 dll so dylib csproj msbuild copytooutputdirectory",
  jvm:"java 자바 kotlin 코틀린 jvm jni 제이엔아이 native 네이티브 javah javac -h System.loadLibrary loadLibrary UnsatisfiedLinkError JNIEnv JavaVM jstring jobject jclass jmethodID jfieldID GetStringUTFChars ReleaseStringUTFChars NewStringUTF modified utf-8 모디파이드 GetPrimitiveArrayCritical DirectByteBuffer NewDirectByteBuffer GetDirectBufferAddress allocateDirect 제로카피 zerocopy 복사없이 panama 파나마 ffm foreign function memory Linker SymbolLookup FunctionDescriptor MemorySegment Arena MethodHandle invokeExact downcallHandle upcallStub jextract jna jnr jnr-ffi libffi Native.load LibraryLoader Structure Callback RegisterNatives JNI_OnLoad JNI_OnUnload AttachCurrentThread DetachCurrentThread ExceptionCheck ThrowNew Xcheck:jni hs_err 크래시 gradle 그래들 cmake externalNativeBuild NativeLoader jar 패키징 packaging android 안드로이드 ndk abiFilters arm64-v8a jniLibs 16kb page cinterop kmp 멀티플랫폼 코루틴 coroutine spring boot 스프링 jmh 벤치마크 benchmark enable-native-access java25 java22 lts 자바모듈 자바바인딩 자바에서c++ c++자바연동 안드로이드ndk",
};
...
const SEC_KW = {
  ...
  g01:"툴킷 선택 비교 qt imgui sdl3 raylib wxwidgets juce slint winui electron tauri 시작속도 메모리 라이선스 lgpl 판단기준 pyside6 대조",
  g02:"설치 온라인설치기 installer 계정 오픈소스 msvc mingw 컴포넌트 vcpkg qtbase cmake findpackage qt6config cmake_prefix_path qtdir cmakepresets 프리셋 automoc autouic qtcreator vscode clangd compile_commands",
  g03:"qapplication qmainwindow 이벤트루프 exec 시그널슬롯 connect 람다 함수포인터 signal slot 부모자식 수명 deleteLater qpointer 레이아웃 vboxlayout hboxlayout stretch qtableview qabstracttablemodel 모델뷰 프록시 정렬 필터 ui파일 designer uic pyside6 대조",
  g04:"qml qtquick 선언형 declarative qqmlapplicationengine loadfrommodule qt_add_qml_module q_property notify q_invokable qml_element qml_singleton qml_uncreatable setcontextproperty 바인딩 listview delegate qquickwidget 씬그래프 scenegraph 성능 프로파일러 widgets비교",
  g05:"qthread movetothread 워커 qtconcurrent run mapped qfuture then qfuturewatcher 진행률 취소 응답없음 프리징 qnetworkaccessmanager qnetworkreply 타임아웃 json qsettings savegeometry qstandardpaths qsqldatabase qsqlquery sqlite wal qsqltablemodel tr lupdate lrelease qtranslator 다국어 i18n",
  g06:"배포 deploy windeployqt macdeployqt linuxdeployqt appimage 플러그인 qwindows platforms 재배포런타임 vcredist 정적빌드 static 라이선스 innosetup nsis wix msi msix qtifw 설치파일 installer 코드서명 signtool codesign notarization 공증 smartscreen 크기줄이기 qt_debug_plugins",
  g07:"imgui dearimgui 즉시모드 immediate 보존모드 retained glfw opengl3 glad vcpkg 백엔드 begin end button sliderfloat begintable listclipper treenode popup docking ini 폰트 한글 ime pushid id충돌 demo window implot imguizmo nuklear",
  g08:"sdl sdl3 sdl2 마이그레이션 sdl_init createwindow renderer pollevent sdl_event_quit 키보드 게임패드 텍스처 오디오 sdl_gpu appiterate 콜백 raylib initwindow begindrawing 게임루프 gameloop 델타타임 프레임 누산기 accumulator fixedstep 터널링 vsync hidpi",
  g09:"wxwidgets wx wxframe wxpanel wxboxsizer sizer bind wxevt 이벤트테이블 skip wxstring fromutf8 callafter wxthread wxconfig wxformbuilder 네이티브 nsbutton gtkbutton win32 mfc 결합 gethandle 라이선스 wxwindows 정적링크 audacity kicad filezilla",
  g10:"win32 winapi wndproc 메시지루프 getmessage dispatchmessage defwindowproc registerclass createwindow dpi 유니코드 트레이 systemtray shell_notifyicon notifyicondata 풍선알림 트레이메뉴 trackpopupmenu qsystemtrayicon 자동시작 run레지스트리 작업스케줄러 뮤텍스 중복실행 qlocalserver 파일연결 progid cppwinrt winrt 코루틴 토스트 winui3 windowsappsdk xaml mica dwmsetwindowattribute",
  g11:"성능 performance 모델뷰 qabstracttablemodel data role 캐시 델리게이트 sizehint resizetocontents uniformrowheights 고정행높이 fetchmore canfetchmore 프록시 정렬 백만행 대용량 mvvm mvc 뷰모델 계층분리 core viewmodel ui 디바운스 debounce 프로파일링 tracy superluminal perf instruments qelapsedtimer 디버그빌드 릴리즈",
  g12:"실전 프로젝트 로그뷰어 logviewer 폴더구조 cmake vcpkg mmap 메모리매핑 인덱싱 lineref 델리게이트 하이라이트 검색 필터 워커 취소 토큰 경쟁 qsettings 드래그앤드롭 단축키 최근파일 명령줄 qcommandlineparser release스크립트 githubactions ci 태그 innosetup 서명 배포",
  k01:"선택 비교 pinvoke dllimport libraryimport c++/cli com cppsharp swig clangsharp 표 흐름도 기본값 크로스플랫폼 nativeaot 소스생성기 il스텁 경계비용 benchmarkdotnet",
  k02:"첫호출 hello dllexport visibility cmake shared dotnet new csproj none update content copytooutputdirectory preservenewest dllnotfoundexception entrypointnotfound dumpbin nm 심볼 nativelibrary 스모크테스트 생성스텁 emitcompilergeneratedfiles",
  k03:"마샬링 blittable 타입대응 int32 int64 long lp64 llp64 size_t nint bool marshalas u1 문자열 utf8 stringmarshalling ptrtostringutf8 버퍼 callingconvention cdecl stdcall setlasterror getlastpinvokeerror disableruntimemarshalling sizeof",
  k04:"구조체 structlayout sequential explicit fieldoffset pack 패딩 정렬 union span readonlyspan fixed stackalloc gchandle pinned nativememory alignedalloc arraypool stride roi writeablebitmap 다차원배열 rowmajor memorydiagnoser 할당0",
  k05:"핸들 safehandle safehandlezeroorminusoneisinvalid releasehandle idisposable dispose using 불투명포인터 opaque 소유권 수명 borrowed owned gc.keepalive objectdisposedexception 종료자 finalizer criticalfinalizer 누수테스트 강타입핸들",
  k06:"콜백 callback 델리게이트 delegate unmanagedcallersonly 함수포인터 delegate* unmanaged unmanagedfunctionpointer gchandle user포인터 gc수거 keepalive 스레드 channel dispatcher 취소 예외삼키기 콜백비용 배치처리",
  k07:"예외 exception 에러코드 errorcode last_error thread_local guard 매크로 catch 봉인 마샬 getlastpinvokeerror getlastpinvokeerrormessage win32exception 세그폴트 sigsegv accessviolation 덤프 dotnet-dump lldb sanitizer 계약테스트 std::expected",
  k08:"c++/cli cppcli clr ref class gcnew 소멸자 종료자 property system::string marshal_as msclr pin_ptr array vcxproj clrsupport netcore anycpu badimageformat 혼합어셈블리 로더락 dllmain pragma managed 점진적이주",
  k09:"자동생성 바인딩 clangsharp clangsharppinvokegenerator swig cppsharp libclang 인터페이스파일 typemap newobject traverse remap rsp 생성물커밋 abi스냅샷 nm 심볼 회귀검사 후처리 어노테이션 owned borrowed",
  k10:"unity 유니티 plugins x86_64 bundle android arm64-v8a ios __internal dllimport il2cpp mono monopinvokecallback aot 스트리핑 link.xml preserve il2cppsetoption nativearray getunsafeptr getrawtexturedata 텍스처 ndk 툴체인 c++_static xcode postprocessbuild loadlibrary freelibrary 에디터재시작",
  k11:"배포 nuget pack packagepath runtimes rid win-x64 linux-x64 osx-arm64 linux-arm64 musl alpine glibc setdllimportresolver nativelibrary tryload moduleinitializer avx512 디스패치 githubactions 매트릭스 singlefile includenativelibrariesforselfextract docker distroless nativeaot publishaot directpinvoke nativelibrary 트리밍 isaotcompatible",
  k12:"실전 프로젝트 이미지 리사이즈 바이리니어 stride 스레드 hardware_concurrency 경계함수 검증 래퍼 span 오버로드 cli skiasharp aspnetcore minimalapi arraypool threads1 unity texture2d getrawtexturedata 저장소구조 monorepo ci benchmarkdotnet memorydiagnoser ratio allocated 경계비용 측정",
  v01:"비교 선택 jni ffm panama jna jnr 성능 오버헤드 안드로이드 graalvm enable-native-access 흐름도 기본값 2026",
  v02:"javac -h javah 헤더생성 JNIEXPORT JNICALL loadLibrary java.library.path find_package JNI cmake fPIC shared dll dylib nm 심볼 UnsatisfiedLinkError",
  v03:"jint jlong jboolean jchar 시그니처 javap -s GetStringUTFChars GetStringChars modified utf-8 cesu-8 이모지 getBytes RAII NewGlobalRef DeleteLocalRef PushLocalFrame 약한전역",
  v04:"GetByteArrayElements GetArrayRegion JNI_ABORT GetPrimitiveArrayCritical DirectByteBuffer allocateDirect NewDirectByteBuffer GetDirectBufferAddress GetDirectBufferCapacity MaxDirectMemorySize 엔디안 버퍼풀 제로카피",
  v05:"JNI_OnLoad FindClass GetMethodID 캐싱 CallVoidMethod 콜백 ThrowNew ExceptionCheck ExceptionClear 핸들패턴 AutoCloseable Cleaner AttachCurrentThreadAsDaemon DetachCurrentThread JavaVM",
  v06:"호출오버헤드 배치 배치호출 RegisterNatives JNINativeMethod Xcheck:jni hs_err SIGSEGV Problematic frame ASan LD_PRELOAD 코어덤프 NativeMemoryTracking jcmd",
  v07:"panama ffm foreign Linker nativeLinker SymbolLookup libraryLookup loaderLookup defaultLookup FunctionDescriptor ValueLayout MemorySegment Arena ofConfined ofShared invokeExact downcallHandle java25 java22",
  v08:"jextract 자동생성 바인딩 dump-includes StructLayout paddingLayout VarHandle SequenceLayout upcallStub 콜백 critical firstVariadicArg 가변인자 한계 안드로이드미지원",
  v09:"jna jnr jnr-ffi Native.load Library Structure getFieldOrder Callback jna-platform LibraryLoader libffi 프록시 프로토타입 마이그레이션 인터페이스분리",
  v10:"gradle cmake Exec task processResources NativeLoader mapLibraryName getResourceAsStream 임시파일 System.load github actions 매트릭스 glibc manylinux musl alpine spring boot 빈 destroyMethod 체크리스트",
  v11:"android 안드로이드 ndk externalNativeBuild cmake abiFilters arm64-v8a armeabi-v7a jniLibs c++_shared 16kb page max-page-size android_log external fun object init 코루틴 Dispatchers 취소 cinterop KMP expect actual",
  v12:"실전 프로젝트 resize_rgba 이중선형 인터페이스 JniImageOps FfmImageOps MemorySegment.ofBuffer jmh 벤치마크 AverageTime Blackhole spring boot RestController 폴백 배포 체크리스트",
};

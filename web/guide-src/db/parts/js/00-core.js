...
const TAB_KW = {
  ...
  redis:"redis 레디스 nosql 노에스큐엘 키값 key value 인메모리 메모리 캐시 cache 캐싱 세션 session ttl expire 만료 자료구조 string hash list set sorted set zset 순위표 랭킹 리더보드 카운터 분산락 lock rate limit 처리량제한 rdb aof 영속성 스냅샷 maxmemory lru lfu 축출 eviction pubsub 발행구독 stream 스트림 큐 sentinel 센티널 클러스터 cluster 복제 keys scan 빅키 핫키 슬로우로그 valkey memcached redis-cli redisinsight 6379",
  mongo:"mongodb 몽고db 몽고디비 nosql 문서형 document 도큐먼트 bson json 컬렉션 collection objectid _id insertone find updateone deleteone updatemany upsert 쿼리연산자 gte lte in exists regex 프로젝션 내장 embed 참조 reference 스키마리스 집계 aggregate 파이프라인 match group lookup unwind project 인덱스 collscan ixscan explain ttl인덱스 텍스트검색 트랜잭션 write concern majority 복제셋 replica set 샤딩 shard 검증 validator jsonschema 16mb mongosh compass atlas pymongo mongoose spring data 27017",
  ...
  olap:"olap oltp 올랩 분석 analytics 컬럼 컬럼지향 컬럼형 columnar column store 행저장 row store clickhouse 클릭하우스 클릭하우스 duckdb 덕디비 덕db parquet 파케이 파켓 arrow 애로우 mergetree 머지트리 머티리얼라이즈드뷰 materialized view 사전집계 aggregatingmergetree replacingmergetree ttl 스킵인덱스 dictionary 딕셔너리 timescaledb 타임스케일 시계열 timeseries influxdb prometheus 프로메테우스 victoriametrics 벡터 vector 벡터검색 vector search 임베딩 embedding pgvector 피지벡터 hnsw ivfflat ann 근사검색 recall 재현율 코사인 cosine 유클리드 내적 halfvec binary quantize 양자화 하이브리드 hybrid bm25 tsvector rrf reciprocal rank fusion 재순위 reranker rerank 크로스인코더 qdrant 큐드런트 milvus weaviate pinecone chroma elasticsearch opensearch rag 라그 검색증강 청킹 chunking 재색인 증분갱신 중복제거 recall@k mrr ndcg 데이터파이프라인 etl elt cdc debezium 데비지움 kafka dbt 레이크하우스 lakehouse iceberg delta 스키마변경 대시보드 데이터웨어하우스 warehouse 집계 groupby 벡터화 simd 압축 zstd snappy 파티셔닝 hive partitioning 로우그룹 rowgroup",
};
...
const SEC_KW = {
  ...
  r01:"nosql이란 rdbms차이 비교 관계형차이 스키마리스 조인없음 수평확장 키값 문서형 와이드컬럼 그래프 cassandra neo4j 언제쓰나 역할분담",
  r02:"redis특징 인메모리 빠른이유 싱글스레드 자료구조서버 마이크로초 지연 캐시왜",
  r03:"redis설치 도커 redis-cli ping 6379 비밀번호 requirepass protected-mode 노출사고 valkey redisinsight",
  r04:"키설계 네이밍 콜론 접두사 ttl expire 유효기간 수명 persist db번호 select scan 순회 일괄삭제 키사전 otp 인증번호",
  r05:"자료구조 string hash list set sorted set zset incr 카운터 최근본상품 ltrim 좋아요 유니크방문자 순위표 zadd zrange bitmap hyperloglog geo hash vs string 선택 부분수정 hincrby setbit bitcount 출석 pfadd pfcount uv 순방문자",
  r06:"캐시 cache aside 패턴 히트 미스 무효화 invalidation ttl 지터 스탬피드 stampede 낡은값 del 네거티브캐시 없음캐시 키버전 지터 jitter",
  r07:"세션저장소 세션공유 순위표 리더보드 rate limit 처리량제한 분산락 set nx px lua redisson",
  r08:"영속성 persistence rdb aof 스냅샷 append only everysec 유실 재시작 백업",
  r09:"maxmemory 메모리한도 축출 eviction lru lfu noeviction 정책 info memory 단편화 fragmentation",
  r10:"pubsub publish subscribe 발행구독 채팅 알림 stream xadd xread 소비자그룹 consumer group ack 큐 kafka대체",
  r11:"복제 replica replicaof sentinel 센티널 자동승격 페일오버 cluster 클러스터 슬롯 16384 고가용성 ha elasticache",
  r12:"keys금지 scan slowlog 빅키 bigkey 핫키 hotkey unlink flushall multi exec 트랜잭션 lua 원자성 히트율 모니터링 pipeline 파이프라인 multi exec lua 원자 재고차감 bigkeys memory usage",
  r13:"redis python redis-py java lettuce jedis spring cacheable node ioredis 파이프라인 타임아웃 폴백",

  n01:"mongodb란 문서형 도큐먼트 컬렉션 필드 _id objectid bson 용어대응 테이블대응 조인대신 내장 언제쓰나",
  n02:"mongodb설치 도커 atlas 아틀라스 무료 mongosh compass 컴퍼스 인증 sspl 라이선스 27017",
  n03:"crud insertone insertmany find updateone updatemany deleteone set inc push unset upsert sql대조 배열 push pull addtoset slice elemmatch 위치연산자 positional",
  n04:"쿼리연산자 gt gte lt lte in or exists 배열검색 점표기법 내장문서 sort skip limit 페이징 countdocuments",
  n05:"문서설계 내장 embed 참조 reference 모델링 16mb 배열 비정규화 화면단위 설계기준",
  n06:"집계 aggregate 파이프라인 match group sum avg having lookup 조인 unwind project group by대응 일별매출 리포트 datetostring 타임존 timezone merge out",
  n07:"인덱스 createindex 복합인덱스 유니크 ttl인덱스 텍스트인덱스 explain collscan ixscan 실행계획 풀스캔 부분인덱스 partialfilterexpression 커버드 covered totaldocsexamined",
  n08:"트랜잭션 원자성 단일문서 멀티도큐먼트 세션 commit abort write concern majority 유실",
  n09:"복제셋 replica set 프라이머리 세컨더리 자동승격 선거 읽기분산 readpreference 샤딩 샤드키 백업 mongodump mongorestore mongoexport 복원 pitr 스냅샷",
  n10:"스키마검증 validator jsonschema 필수필드 enum 안티패턴 무한배열 16mb 필드명데이터 스키마버전",
  n11:"pymongo beanie mongoengine spring data mongodb mongorepository node 드라이버 mongoose nosql인젝션 sanitize",
  n12:"폴리글랏 같이쓰기 병행 역할분담 jsonb비교 postgresql대신 언제mongodb 언제rdbms 선택기준",
  ...
  v01:"oltp olap 행저장 컬럼저장 columnar row store 벡터화 simd 압축 사전인코딩 delta 인코딩 seq scan 읽는양 io 절감 clickhouse postgresql 비교 분석db 도입기준 읽기복제본",
  v02:"duckdb 덕디비 설치 pip install cli read_parquet read_csv read_json_auto 메모리모드 파일모드 pandas polars arrow 제로카피 attach postgres s3 httpfs 디스크스필 read_only 동시쓰기 columns exclude replace group by all",
  v03:"parquet 파케이 행그룹 rowgroup 컬럼청크 페이지 푸터 footer 통계 minmax zone map predicate pushdown 파티션프루닝 hive partitioning zstd snappy gzip lz4 딕셔너리인코딩 rle 델타인코딩 arrow 제로카피 parquet_metadata 작은파일문제",
  v04:"clickhouse 클릭하우스 docker 설치 clickhouse-client clickhouse-local mergetree order by 정렬키 primary key partition by toyyyymm index_granularity 그래뉼 희소인덱스 파트 병합 merge insert 배치 async_insert s3 postgresql 함수 lowcardinality uniq quantile explain indexes",
  v05:"머티리얼라이즈드뷰 materialized view aggregatingmergetree replacingmergetree summingmergetree collapsingmergetree state merge sumstate uniqstate quantilestate ttl 보관정책 다운샘플링 스킵인덱스 bloom_filter tokenbf minmax 딕셔너리 dictget 조인 메모리 too_many_parts system.parts query_log mutations final argmax",
  v06:"시계열 timeseries timescaledb 하이퍼테이블 hypertable 청크 chunk create_hypertable time_bucket gapfill locf 연속집계 continuous aggregate 압축정책 보관정책 retention prometheus promql influxdb victoriametrics 카디널리티 라벨폭발 iot 센서 메트릭",
  v07:"임베딩 embedding 벡터 vector 차원 dimension 코사인 cosine 유클리드 l2 내적 inner product 정규화 normalize brute force knn ann 근사검색 hnsw 계층그래프 ef m ivfflat lists probes 클러스터 재현율 recall qps sentence-transformers bge-m3 openai text-embedding-3",
  v08:"pgvector create extension vector vector(1536) hnsw ivfflat m ef_construction ef_search lists probes 거리연산자 코사인 l2 내적 인덱스안탐 explain seq scan 순차스캔 maintenance_work_mem concurrently halfvec binary_quantize bit 해밍 sparsevec matryoshka 재현율측정 mvcc vacuum reindex",
  v09:"하이브리드검색 hybrid search bm25 tf-idf tsvector ts_rank_cd plainto_tsquery gin rrf reciprocal rank fusion 순위융합 후보수 사후필터링 post-filtering 사전필터링 부분인덱스 partial index 재순위 reranker cross-encoder bge-reranker 검색품질",
  v10:"벡터db qdrant 큐드런트 milvus 밀버스 weaviate 위비에이트 pinecone 파인콘 chroma 크로마 elasticsearch opensearch dense_vector knn retriever rrf payload index 사전필터링 멀티테넌트 관리형 saas 벤더종속 벤치마크 ann-benchmarks 추상화 이전",
  v11:"rag 검색증강생성 스키마설계 doc chunk 청킹 chunking 재귀분할 구조기반 헤딩경로 겹침 overlap 토큰 tiktoken 부모자식 증분갱신 해시 sha256 변경감지 중복제거 재색인 임베딩버전 embed_model 평가세트 recall@k mrr ndcg 비용 임베딩비용 저장비용 프롬프트토큰",
  v12:"데이터파이프라인 etl elt 배치추출 워터마크 증분추출 cdc change data capture debezium 데비지움 wal logical replication 복제슬롯 publication kafka 토픽 replacingmergetree 멱등 idempotent dbt ref incremental lineage 레이크하우스 lakehouse iceberg delta lake hudi 타임트래블 스키마진화 스키마레지스트리 정합성 대조배치",
  v13:"실전프로젝트 사내문서검색 rag 하이브리드 rrf 권한 acl distinct on doc_chunk hnsw gin search_log clickhouse aggregatingmergetree 머티리얼라이즈드뷰 대시보드 무결과율 zero hit ctr p95 duckdb parquet 덤프 성능수치 재색인 운영사례 트러블슈팅 도입순서",
};

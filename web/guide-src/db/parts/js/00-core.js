...
const SEC_KW = {
  ...
  r04:"키설계 네이밍 콜론 접두사 ttl expire 유효기간 수명 persist db번호 select",
  r05:"자료구조 string hash list set sorted set zset incr 카운터 최근본상품 ltrim 좋아요 유니크방문자 순위표 zadd zrange bitmap hyperloglog geo",
  r06:"캐시 cache aside 패턴 히트 미스 무효화 invalidation ttl 지터 스탬피드 stampede 낡은값 del",
  ...
  r12:"keys금지 scan slowlog 빅키 bigkey 핫키 hotkey unlink flushall multi exec 트랜잭션 lua 원자성 히트율 모니터링",
  ...
  n03:"crud insertone insertmany find updateone updatemany deleteone set inc push unset upsert sql대조",
  ...
  n06:"집계 aggregate 파이프라인 match group sum avg having lookup 조인 unwind project group by대응",
  n07:"인덱스 createindex 복합인덱스 유니크 ttl인덱스 텍스트인덱스 explain collscan ixscan 실행계획 풀스캔",
  n08:"트랜잭션 원자성 단일문서 멀티도큐먼트 세션 commit abort write concern majority 유실",
  n09:"복제셋 replica set 프라이머리 세컨더리 자동승격 선거 읽기분산 readpreference 샤딩 샤드키",

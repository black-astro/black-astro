...
const TAB_KW = {
  ...
  ktor:"ktor 크토어 코틀린서버 서버프레임워크 jetbrains netty cio embeddedserver enginemain application.conf hocon 라우팅 routing get post route resources 타입안전라우팅 플러그인 plugin install 파이프라인 pipeline applicationcall contentnegotiation calllogging cors compression statuspages defaultheaders createapplicationplugin kotlinx serialization 직렬화 json receive respond 검증 konform 코루틴 suspend 구조적동시성 coroutinescope async withtimeout dispatcher 취소 exposed dsl dao hikaricp 트랜잭션 newsuspendedtransaction flyway 마이그레이션 r2dbc jooq 인증 authentication jwt 세션 sessions 쿠키 oauth bcrypt argon2 권한 websocket 웹소켓 sse 실시간 채팅 백프레셔 하트비트 koin di 의존성주입 테스트 testapplication mockk testcontainers 관측 mdc 요청id micrometer prometheus 헬스체크 graceful shutdown 배포 shadow fatjar 도커 docker graalvm nativeimage 시작시간 메모리 kubernetes 프로브 스프링비교 spring boot 차이 가벼운서버",
};
...
const SEC_KW = {
  ...
  kt01:"ktor 소개 왜쓰나 스프링비교 spring boot 차이 시작시간 메모리 의존성주입 러닝커브 생태계 엔진 netty cio jetty tomcat 선택기준 jetbrains 경량 마이크로서비스",
  kt02:"프로젝트생성 start.ktor.io intellij 플러그인 gradle 수동설정 embeddedserver enginemain application.conf hocon 폴더구조 핫리로드 autoreload 첫라우트 실행",
  kt03:"라우팅 routing get post put delete route 중첩 경로파라미터 pathparameter 쿼리 queryparameters resources 타입안전 확장함수 모듈분리 정적파일 staticfiles",
  kt04:"플러그인 plugin install 파이프라인 pipeline 페이즈 phase applicationcall contentnegotiation calllogging cors compression statuspages defaultheaders 순서 createapplicationplugin 커스텀플러그인 인터셉터",
  kt05:"kotlinx serialization 직렬화 serializable json contentnegotiation receive respond 요청본문 응답 검증 validation konform 에러응답 statuspages sealed 결과타입 표준화",
  kt06:"코루틴 coroutine suspend 핸들러 구조적동시성 coroutinescope async await 병렬호출 withtimeout 타임아웃 dispatcher 취소 cancellation 클라이언트끊김 webflux 비교 mvc 비교",
  kt07:"exposed dsl dao 테이블정의 쿼리 join hikaricp 커넥션풀 트랜잭션 transaction newsuspendedtransaction 코루틴 flyway 마이그레이션 r2dbc jooq 비교 postgresql",
  kt08:"인증 authentication 인가 authorization jwt 발급 검증 리프레시 세션 sessions 쿠키 암호화 oauth 소셜로그인 권한 role 라우트보호 authenticate bcrypt argon2 비밀번호해시",
  kt09:"websocket 웹소켓 sse serversentevents 실시간 채팅 브로드캐스트 세션집합 동시성 concurrenthashmap mutex 백프레셔 하트비트 핑퐁 연결종료 재연결",
  kt10:"koin di 의존성주입 모듈 single factory 수동di 생성자주입 테스트 testapplication 통합테스트 요청검증 mockk 목킹 testcontainers postgres ci",
  kt11:"관측 observability calllogging mdc 요청id traceid 구조화로그 micrometer prometheus 메트릭 헬스체크 health graceful shutdown 정상종료 설정분리 hocon 환경변수 프로파일",
  kt12:"배포 deploy shadow fatjar installdist 도커 dockerfile 멀티스테이지 jre 슬림 이미지크기 graalvm nativeimage 한계 시작시간 메모리실측 kubernetes 프로브 readiness liveness",
  kt13:"실전 프로젝트 짧은url 단축 통계 폴더구조 라우팅 exposed postgresql redis 캐시 카운터 jwt websocket 실시간 테스트 docker compose 배포 스프링비교 시작시간 메모리",
};
let FIDX = null;

...
const TAB_KW = {
  ...
  log:"로그 log 로깅 logging append-only 어펜드온리 추가만 순서있는기록 ordered sequence 오프셋 offset 세그먼트 segment 큐 queue 카프카 kafka 이벤트소싱 event sourcing 이벤트 event 상태 state fold 접기 미분 derivative CRUD 스냅샷 snapshot 복기 replay 재생 업캐스터 회계 복식부기 역분개 WAL write-ahead 선행기록 원자성 atomicity 커밋레코드 fsync 크래시복구 recovery ARIES Analysis Redo Undo 체크포인트 checkpoint 멱등 idempotent 저널 journal ext4 NTFS PostgreSQL Redis AOF Git etcd Raft 논리시계 logical clock 램포트 Lamport 벡터시계 vector clock happens-before 인과 causality HLC 하이브리드 TrueTime 벽시계 시계역전 시퀀서 sequencer 합의 consensus 상태기계복제 RSM state machine replication 리더 팔로워 과반 quorum ZooKeeper Consul KRaft CockroachDB TiKV 분산락 리더선출 CQRS 읽기모델 CDC Debezium 데이터베이스뒤집기 turning database inside out 파티션 partition 순서보장 결과적일관성 정보이론 information theory 엔트로피 채널 카디널리티 cardinality 메트릭 metric 레이블 label 카디널리티폭발 관측가능성 observability 제어이론 넓은이벤트 wide event 구조적로깅 structured logging JSON 필드 스키마 schema 스키마진화 호환성 backward forward full Avro Protobuf 필드번호 매핑충돌 mapping conflict OpenTelemetry OTel semantic conventions ECS ObservedTimestamp 컴팩션 compaction 로그컴팩션 tombstone 툼스톤 보존 retention TTL LSM SSTable 티어링 tiering 콜드스토리지 GDPR 잊힐권리 암호파쇄 crypto shredding 무결성 integrity 해시체인 hash chain 머클트리 Merkle tree 포함증명 inclusion proof 일관성증명 consistency proof 인증서투명성 Certificate Transparency CT Rekor Trillian sigstore 변조방지 변조감지 tamper-evident WORM 감사로그 audit log 관측자효과 사고조사 포스트모템 postmortem 인과추론 상관관계 로그의한계 증거",
};
...
const SEC_KW = {
  ...
  lg01:"로그 정의 append-only 추가만 ordered sequence 순서있는기록 연산 append read offset 오프셋 update delete insert 없는연산 불변 immutable 순차쓰기 sequential write 랜덤쓰기 세그먼트 segment 인덱스 큐 queue 차이 소비자 consumer 재생 replay ext4 NTFS 저널 PostgreSQL WAL Redis AOF Git Kafka etcd Raft 블록체인 journald 오프셋재사용",
  lg02:"상태 state 이벤트 event 이중성 duality fold 접기 미분 derivative apply 순수함수 pure function 결정성 determinism 현재시각 난수 외부API 이벤트소싱 event sourcing CRUD 스냅샷 snapshot 복기 replay 워킹트리 복제본 재무제표 회계 복식부기 역분개 감사 audit 되돌리기 이유",
  lg03:"WAL write-ahead logging 선행기록 원자성 atomicity 커밋레코드 commit record fsync 디스크확정 durability 크래시복구 crash recovery ARIES Analysis Redo Undo 체크포인트 checkpoint 멱등 idempotent LSN 페이지 page 순차 sequential rename 임시파일 atomic rename 트랜잭션",
  lg04:"시간 time 벽시계 wall clock NTP 시계역전 clock skew 단조시계 monotonic 논리시계 logical clock 램포트 Lamport 벡터시계 vector clock happens-before 인과 causality 동시 concurrent 부분순서 partial order 전순서 total order 시퀀서 sequencer 합의 consensus HLC 하이브리드논리시계 TrueTime Spanner commit wait traceId spanId 로그정렬 last-write-wins LWW",
  lg05:"합의 consensus 복제로그 replicated log 상태기계복제 RSM state machine replication 결정성 determinism Raft Paxos 리더 leader 팔로워 follower 과반 quorum majority 로그매칭 log matching 커밋인덱스 commitIndex 오프셋 fsync 지연 latency etcd ZooKeeper Consul Kafka KRaft CockroachDB TiKV 분산락 리더선출 메타데이터 비용",
  lg06:"로그기반아키텍처 turning database inside out 이벤트소싱 event sourcing CQRS 명령 command 조회 query 읽기모델 read model projection 프로젝션 재생 replay CDC change data capture Debezium 아웃박스 outbox 듀얼라이트 dual write 카프카 Kafka 파티션 partition 순서보장 키 key 결과적일관성 eventual consistency read-your-writes 구독 subscribe 결합도 coupling",
  lg07:"정보이론 information theory 엔트로피 entropy 채널 channel 정보량 카디널리티 cardinality 고카디널리티 high cardinality 조합폭발 곱셈 메트릭 metric 레이블 label 시계열 time series 카디널리티폭발 사전집계 pre-aggregation 사후질문 희귀사건 rare event 꼬리 tail p99 샘플링 sampling 설계",
  lg08:"관측가능성 observability 제어이론 control theory 관측행렬 observability matrix 판별불가 unidentifiable 내부상태 출력 output 메트릭 metric 로그 log 트레이스 trace 넓은이벤트 wide event 사전집계 pre-aggregation 카디널리티 계측 instrumentation 설계 스키마 필드추가 배포 OpenTelemetry OTel 샘플링",
  lg09:"구조적로깅 structured logging JSON 레코드 record 필드 field 이벤트이름 event name 규약 convention slog Logstash 인코더 encoder 스키마 schema 스키마진화 evolution 호환성 compatibility 후방 backward 전방 forward 완전 full Avro Protobuf 필드번호 reserved 매핑충돌 mapping conflict Elasticsearch 색인거부 ECS OpenTelemetry semantic conventions Timestamp ObservedTimestamp 업캐스터 upcaster 버전",
  lg10:"컴팩션 compaction 로그컴팩션 log compaction 툼스톤 tombstone 보존 retention TTL 시간기반 키기반 스냅샷 snapshot 잘라내기 truncate Raft LSM SSTable 병합 merge Git gc WAL 체크포인트 저장소계층 tiering 콜드스토리지 S3 비용 GDPR 잊힐권리 삭제 암호파쇄 crypto shredding 복제본 백업",
  lg11:"무결성 integrity 변조 tamper 변조감지 tamper-evident 변조방지 tamper-proof 해시체인 hash chain 해시 SHA-256 머클트리 Merkle tree 루트해시 포함증명 inclusion proof 일관성증명 consistency proof 앵커링 anchoring 절단공격 truncation 인증서투명성 Certificate Transparency CT SCT 감시자 monitor 감사자 auditor Rekor Trillian sigstore 공급망 supply chain WORM ObjectLock 감사로그 audit log 서명",
  lg12:"로그의한계 limitation 증거 evidence 진실 truth 생존편향 survivorship bias 부재 absence 닿지않은요청 인과 causality 상관관계 correlation 관측자효과 observer effect 계측비용 로거실패 유실 drop 버퍼 사고조사 incident 포스트모템 postmortem 롤백 카나리 canary 실험 두번째출처 second source 메트릭 트레이스 대조",
};
let FIDX = null;

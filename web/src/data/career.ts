...
export const projects: CareerProject[] = [
  {
    ...
    sections: [
      ...
      {
        heading: '적재 구조 정비 (인덱스 제어 + BATCH 청크)',
        points: [
      ...
      {
        heading: 'SQL 튜닝으로 먼저 시도한 것 (현재는 쓰지 않는 경로)',
        points: [
          ...
          '판단 — 시간은 줄었지만 자릿수가 바뀌지 않았다. SQL을 더 다듬는 방향으로는 한계라고 보고 이 경로를 버렸다. 아래 구조 재설계로 대체했고 현재 운영에서는 이 프로시저를 호출하지 않는다.',
        ],
      },
      {
        heading: '중복제거 아키텍처 재설계 — 튜닝의 한계를 구조 변경으로 (전 구간 약 31분)',
        points: [
          '문제 정의 — 개선 전에는 적재(INSERT)에 50분, 중복제거 프로시저에 4시간이 걸렸다. 프로시저를 아무리 다듬어도 자릿수는 안 바뀐다고 보고, 병목을 연산의 위치 문제로 다시 정의. 원천 1,000만 행을 먼저 적재한 뒤 그 넓은 테이블에서 정렬·집계를 반복하는 순서 자체가 비용이었다.',
    ...
    outcome:
      '중복 판정을 적재 이전의 로컬 SQLite로 옮기는 아키텍처 재설계로, 적재 50분 + 중복제거 프로시저 4시간이던 구간을 전 구간 약 31분으로 단축. 적재 구간은 파일당 44초 → 0.8초 (운영 동일 규모 실데이터, 개발 환경 실측). 회차당 DB INSERT 약 1,000만 row 처리, 대용량 XML OOM 0건(운영 로그 기준). 정합성 검증 완료 후 운영 전환 준비 중.',
  },
  {
    id: 'gibisbiz',
    index: 2,

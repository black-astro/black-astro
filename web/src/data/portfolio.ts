...
export const caseStudies: CaseStudy[] = [
  {
    ...
    tag: '대용량 · DB 튜닝',
    summary:
      '델파이 레거시를 Java 21로 재구축. 규모별 파싱 전략 분리와 파서 풀 + 단일 라이터로 기반을 잡고, 프로시저 튜닝이 한계에 닿자 중복 판정을 적재 이전으로 옮기는 구조 재설계로 전 구간을 4시간대에서 30분대로 단축.',
    stack: ['Java 21', 'Spring Boot 3.4.5', 'Tibero 6', 'StAX/JAXB', 'MyBatis(BATCH)', 'SQLite'],
    metrics: [
      { value: '4시간+ → 31분', label: '적재~중복제거 전 구간' },
...
SELECT SERVER_NO, DEDUP_CI
 ...
 WHERE PROC_STATUS = 'N'      -- 선두 등치 조건: 완료 행은 진입 자체 배제
   AND SERVER_NO   = :serverNo;`,
      ...
      {
        type: 'diagram',
        heading: '해결 4 — 튜닝의 한계에서 구조 변경으로 (전 구간 4시간+ → 약 31분)',
        content: `프로시저를 5시간→2시간까지 줄였지만 전 구간은 여전히 4시간 초과.

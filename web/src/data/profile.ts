...
export const achievements: Achievement[] = [
  ...
  },
  {
    ...
    detail: 'KT_BatchServer — 프로시저를 집합 기반으로 바꿔 5시간→2시간까지 줄였지만 한 회차 전 구간은 4시간을 넘었다. 병목을 연산의 위치 문제로 재정의하고, 중복 판정에 필요한 컬럼이 네 개뿐이라는 점에 착안해 판정을 적재 이전 단계의 로컬 SQLite로 옮겨 윈도우 함수 한 번(20초)으로 처리. 사후 UPDATE 조인까지 사라져 전 구간 약 31분. 잔여 병목은 스레드 덤프 샘플링과 구간별 계측으로 좁혀 적재 파일당 44초 → 0.8초(운영 동일 규모 실데이터, 개발 환경 실측).',
  },
  {
    metric: '3',
    unit: '채널',
